...
- (void)close
{
    ...
    */
    ...
    if (rc != SQLITE_OK) {
        Log(@"error finalizing for query: %@ (#%d)", [self query], rc);
    }
    
    pStmt = nil;
	
	//Log(@"0x%08X RS: CLOSE: %@", self, [self query]);
}
...
- (void)setupColumnNames {
    ...
    for (columnIdx = 0; columnIdx < columnCount; columnIdx++) {
        [columnNameToIndexMap setObject:[NSNumber numberWithInt:columnIdx]
                                 forKey:sqlite3_column_name(pStmt, columnIdx) ? [[NSString stringWithUTF8String:sqlite3_column_name(pStmt, columnIdx)] lowercaseString] : @"(null)"];
    }
}
...
- (BOOL)next
{
	...
	do
	{
		...
		{
			Log(@"Throttling on %@ (ATResultSet, SQLITE_BUSY)", query);
			debug_throttle = YES;
		}
...
- (int)columnIndexForName:(NSString*)columnName {
    ...
    Log(@"Warning: I could not find the column named '%@' (columns = %@).", columnName, columnNameToIndexMap);
...
- (void)setPStmt:(sqlite3_stmt *)value {
    pStmt = value;
	//Log(@"0x%08X RS:  OPEN: %@", self, [self query]);
}
